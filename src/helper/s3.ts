...
export const file_upload_response = async (req, res) => {
    reqInfo(req)
    try {
...
export const upload_csv_file = async(csvData) => {
    return new Promise(async function (resolve, reject) {
        try {
           
    const params = {
      Bucket: bucket_name, // your bucket name
      Key: `${Date.now()}-inventory-data.csv`, // Date.now() is use for creating unique file name
      ACL: 'public-read',
      Body: csvData,
...
export const upload_invoice_pdf = async(fileContent ,invoice) =>
{
    return new Promise(async function (resolve, reject) {
        try {
            var params = {
